...
implementation of the model for controlled ODE-RNN
"""
...
def save_checkpoint(model, optimizer, path, epoch):
    ...
    :param model: a torch model, e.g. instance of CODERNN
...
class CODERNN(torch.nn.Module):
    ...
    def __init__(
        ...
        :param options: kwargs, used: kword "options" with arg a dict passed
                from train.train (kwords: 'which_loss', 'residual_enc_dec' are
                used)
        """
        super().__init__()
    ...
    def evaluate(self, times, time_ptr, X, obs_idx, delta_t, T, start_X, 
                 n_obs_ot, cond_exp_fun_kwargs,
                 diff_fun=torch.nn.MSELoss(reduction='mean'), 
                 return_paths=False):
        """
        ...
        _, _, path_t, path_h, path_y = self.forward(
            ...
            return_path=True, get_loss=False, until_T=True)

        stockmodel = data_utils._STOCK_MODELS[
            cond_exp_fun_kwargs['model_name']](**cond_exp_fun_kwargs)
        loss, true_path_t, true_path_y = stockmodel.compute_cond_exp(
            times, time_ptr, X, obs_idx, delta_t, T, start_X, n_obs_ot,
            return_path=True, get_loss=False, **cond_exp_fun_kwargs)
        eval_loss = diff_fun(path_y, true_path_y)
        if return_paths:
            return eval_loss, path_t, true_path_t, path_y, true_path_y
        else:
            return eval_loss
        
    def get_pred(self, times, time_ptr, X, obs_idx, delta_t, T,
                          start_X):
        """
        get predicted path
        :param times: see forward
